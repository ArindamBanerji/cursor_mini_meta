    ...
    def test_get_error_logs_with_filters(self, monitor_service, mock_state_manager):
        ...
        logs = [
            ...
            },
            {
                ...
                "message": "Error 2",
                "timestamp": (now - timedelta(hours=1, minutes=59)).isoformat(),  # Just under 2 hours
        ...
        # Expected 2 logs within the last 2 hours (Error 2 and Error 3)
        assert len(time_filtered) == 2, f"Expected 2 logs within last 2 hours, got {len(time_filtered)}: {[l.message for l in time_filtered]}"
        
        # Make sure logs are ordered by timestamp (newest first)
        assert [log.message for log in time_filtered] == ["Error 3", "Error 2"]
        ...
        assert [log.message for log in type_filtered] == ["Error 3", "Error 1"]
        
        # Test filtering by component
        component_filtered = monitor_service.get_error_logs(component="component_a")
        assert len(component_filtered) == 2
        # Verify logs are ordered by timestamp (newest first)
        assert [log.message for log in component_filtered] == ["Error 3", "Error 1"]
