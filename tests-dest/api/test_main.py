...
class TestMainApp:
    ...
    @pytest.mark.asyncio
    @patch('service_initializer.get_monitor_service')
    async def test_shutdown_event_logs_shutdown(self, mock_get_monitor):
        ...
        args, kwargs = mock_monitor.log_error.call_args
        assert args[0] == "system"
        assert "Application shutdown" in args[1]
        assert kwargs.get("component") == "main"
